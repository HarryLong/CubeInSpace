 ...
 ******************************/
class ControllerWidgetsWrapper : public QObject{
    Q_OBJECT
public:
    enum Type {
        _LATITUDE = 0,
    ...
    ControllerWidgetsWrapper(QAction * latitude_trigger_action, QAction * time_of_day_trigger_action,
                   QAction * month_of_year_trigger_action, QWidget * parent);
    ~ControllerWidgetsWrapper();
...
private slots:
    void trigger_time_overlay(bool checked);
    void trigger_month_overlay(bool checked);
    void trigger_latitude_overlay(bool checked);

private:
    struct ActionAndWidgetPair{
    public:
        ActionAndWidgetPair(QAction * action, BaseControllerWidget * widget) : action(action), widget(widget) {}
        QAction * action;
        BaseControllerWidget * widget;
    };

    std::map<Type, ActionAndWidgetPair> m_widgets;
    std::map<int, std::vector<ActionAndWidgetPair> > m_alignment_sorted_widgets;
};
