...
bool AccelerationSphere::intersects(const glm::vec3 & start, const glm::vec3 & direction) const
{
    float scaler, distance;
    Geom::rayPointDist(start, direction, m_center, scaler, distance) ;
...
std::vector<AccelerationSphere> Base::get_intersecting_spheres(const glm::vec3 & start, const glm::vec3 & direction)
{
    ...
    for(int i = 0; i < m_spheres.size(); i++)
    {
        ...
        if(sphere.intersects(start, direction))
        {
...
std::vector<AccelerationSphere> Hierarchical::get_intersecting_spheres(AccelerationTree & parent, const glm::vec3 & start, const glm::vec3 & direction)
{
    std::vector<AccelerationSphere> intersecting_spheres;

    if(parent.m_node.intersects(start, direction))
    {
        ...
        else
        {
            ...
            {
                std::vector<AccelerationSphere> child_intersecting_spheres(get_intersecting_spheres(child, start, direction));
...
std::vector<AccelerationSphere> Hierarchical::get_intersecting_spheres(const glm::vec3 & start, const glm::vec3 & direction)
{
    return get_intersecting_spheres(*m_acceleration_tree, start, direction);
}
